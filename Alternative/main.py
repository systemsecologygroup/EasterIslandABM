...
class Model:
    """
    Alternative Implementation of the splitting process in the update procedure.
    """
    def __init__(self, _folder, _seed, params_const, params_sensitivity, params_scenarios):
        """
        ...
        self.droughts_rano_raraku = params_const[
            "droughts_rano_raraku"]  # Droughts of Rano Raraku: list of droughts with tuple of start and end date
        ...
        self.map = Map(self, "Map/elevation_EI.tif", "Map/slope_EI.tif", "Map/puleston2017.jpg", bbox_el_image, puleston_bbox)
    ...
    def mu_mean(self, s):
        ...
        Idea:
            We define a mean growth rate $\mu_{\rm m}^{\rm i}(t)$, that determines the stochastic probability of
            each individual in the household $i$ to reproduce or die in each year $t$:
            If $\mu_{\rm m}^{\rm i}(t)>1$ (or $\mu_{\rm m}^{\rm i}(t)<1$), the probability to reproduce (die) is $|\mu_{\rm m}^{\rm i}(t)-1|$ for all individuals of household $i$.
            If $\mu_{\rm m}^{\rm i}(t)=1$, the population size remains constant.
            I.e., on average, the household's population size follows:
            $$ \overline{\frac{dp^{\rm i}(t)}{dt}} = \mu^{\rm i}_{\rm mean}(t) \cdot p^{\rm i}(t) $$

        Calculation:
            We assume, that the (past-dependent) satisfaction, $S^{\rm i}(t)$, fully determines the mean growth rate of agent $i$ at time $t$ and the following trivial statements apply:
            Fully satisfied agents ($S^{\rm i}(t)=1$, i.e.\ both resource requirements have been filled over the past two years) grow at the maximum rate, $\mu_{\rm m, \ max}$, on average, while fully unsatisfied agents ($S^{\rm i}(t)=0$, i.e.\ at least one resource has lacked entirely over the past two years) have growth rate $\mu_{\rm m}(t) = 0$ and, thus, die.
            There is an equilibrium satisfaction, $0<S_{\rm equ}<1$, at which the household's size remains constant.
            By interpreting $S^{\rm i}(t)$ as the `food ratio' in [Puleston2017], we adopt their result that the population size remains stable at $S_{\rm equ} = 0.688$.
            For simplicity, we assume linear relations of the growth rate for the regimes of population decline ($0 \leq S^{\rm i}(t)\leq 0.688$) and growth ($0.688 < S^{\rm i}(t) \leq 1\}$), see Figure \ref{fig:mu}:
            $$ m_{\rm decl} =  \frac{1-0}{S_{\rm equ}-0} ; \quad m_{\rm grow} =  \frac{1.007-1}{1-S_{\rm equ}} $$
            $$ \mu_{\rm m}^{\rm i}(t) = \begin{cases} m_{\rm decl}\cdot S^{\rm i}(t) & \text{if } S^{\rm i}(t) \leq S_{\rm equ} \\ m_{\rm grow}\cdot \left(S^{\rm i}(t)-S_{\rm equ}\right) +1 &  \text{else} \end{cases} $$

        Parameters
        ----------
        s : float between 0 and 1
            the (past) satisfaction of an agent

        Returns
        -------
        mu_mean: float
            mean growth rate mu_mean
        """
        if s >= self.s_equ:
            m_grow = (self.max_p_growth_rate - 1) / (1 - self.s_equ)
            mu_mean = m_grow * (s - self.s_equ) + 1
        else:
            m_decl = 1 / self.s_equ
            mu_mean = m_decl * s
        return mu_mean
    ...
    def P_cat(self, x, cat, infty_penalty=False, **kwargs):
        ...
        **kwargs :
        ...
        penalties = 1 / (1 + np.exp(-k * (x.astype(float) - 0.5 * (x99 + x01))))
...
if __name__ == "__main__":
    print("RUN: ", sys.argv)
    print("Model should be run by providing: ")
    print("   python main.py param_sa_file param_scenario_file seed")
    if len(sys.argv) < 4:
    ...
    const_file = "alternative_consts"  # "default_consts"  # "default_consts"  # "single_agent_consts" "alternative_consts"
    ...
    m.run()
    print("Data and Files are stored in " + folder)
